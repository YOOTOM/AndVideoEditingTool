...
import com.anibear.andvideoeditingtool.videoTrimmer.utils.DisplayUtil;
import com.anibear.andvideoeditingtool.videoTrimmer.utils.SelectViewBean;
...
public class VideoEditProgressView extends RelativeLayout {
    ...
    private LinearLayout llPlayVideoView2;
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        discreteScrollView = new DiscreteScrollView(context, attrs);
        ...
        //discreteScrollView.setLayoutParams(params);
        addView(discreteScrollView, videoEditParams);


        /*llPlayVideoView2 = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.ll_play_video_view, null);  //添加llPlayVideoView
        RelativeLayout.LayoutParams rlParams2 = new RelativeLayout.LayoutParams(DisplayUtil.dipToPx(context, 60), DisplayUtil.dipToPx(context, 60));
        rlParams2.addRule(RelativeLayout.CENTER_VERTICAL, RelativeLayout.TRUE);
        rlParams2.addRule(RelativeLayout.ALIGN_PARENT_RIGHT, RelativeLayout.TRUE);
        addView(llPlayVideoView2, rlParams2);*/
...
//            minScrollWidth = screenWidth / 2 - width;
    ...
    @SuppressLint("ClickableViewAccessibility")
    public void initScrollView(@Nullable RecyclerView.Adapter adapter, @NonNull DiscreteScrollView.OnItemChangedListener<?> onItemChangedListener, @NonNull DiscreteScrollView.ScrollStateChangeListener<?> scrollStateChangeListener) {
        ...
        discreteScrollView.addScrollStateChangeListener(scrollStateChangeListener);
    }

    int count = 0;
    public void setProgress(boolean playState, float currentTime, float totalTime) {
        /*if (playState) {
            Log.e(TAG, "perTotalTime:" + totalTime);
            if (totalTime != 0) {
                final float perX;
                if (totalTime > 18) {
                    perX = screenWidth / (totalTime * 8);
                } else if (totalTime > 16) {
                    perX = screenWidth / (totalTime * 8);
                } else {
                    perX = screenWidth / 160;
                }
                toX = (float) (getX() - 4 * (perX * 0.1));
                setX(count);
                if (toX >= minScrollWidth && playState) {
                    setX(toX);
                    if (playStateListener != null) {
                        playStateListener.videoProgressUpdate((int) currentTime, true);
                    }
...
//        editBarRight.setY(0);
    ...
    public void togglePlayVideo(final boolean playState, ArrayList<BaseImageView> baseImageViews) {
