...
public class RangeSeekBarView extends View {
    ...
    public void initMaxWidth() {
        mMaxWidth = mThumbs.get(1).getMPos() - mThumbs.get(0).getMPos();

        onSeekStop(this, 0, mThumbs.get(0).getMVal());
        onSeekStop(this, 1, mThumbs.get(1).getMVal());
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mFirstRun) {
            for (int i = 0; i < mThumbs.size(); i++) {
                ...
                th.setMVal(mScaleRangeMax * i);
                th.setMPos(mPixelRangeMax * i);
            }
    ...
    public boolean onTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mThumb.setMLastTouchX(coordinate);
                onSeekStart(this, currentThumb, mThumb.getMVal());
                return true;
            }
            case MotionEvent.ACTION_UP: {
                ...
                mThumb = mThumbs.get(currentThumb);
                onSeekStop(this, currentThumb, mThumb.getMVal());
                return true;
            }
            ...
            case MotionEvent.ACTION_MOVE: {
                try {
                    ...
                    final float dx = coordinate - mThumb.getMLastTouchX();
                    final float newX = mThumb.getMPos() + dx;
                    if (currentThumb == 0) {

                        if ((newX + mThumb.getMWidthBitmap()) >= mThumb2.getMPos()) {
                            mThumb.setMPos(mThumb2.getMPos() - mThumb.getMWidthBitmap());
                        } else if (newX <= mPixelRangeMin) {
                            mThumb.setMPos(mPixelRangeMin);
                        } else {
                            //Check if thumb is not out of max width
                            checkPositionThumb(mThumb, mThumb2, dx, true);
                            // Move the object
                            mThumb.setMPos(mThumb.getMPos() + dx);
                        ...
                        if (newX <= mThumb2.getMPos() + mThumb2.getMWidthBitmap()) {
                            mThumb.setMPos(mThumb2.getMPos() + mThumb.getMWidthBitmap());
                        } else if (newX >= mPixelRangeMax) {
                            mThumb.setMPos(mPixelRangeMax);
                        } else {
                            //Check if thumb is not out of max width
                    ...
                    setThumbPos(currentThumb, mThumb.getMPos());
    ...
    private void checkPositionThumb(@NonNull Thumb mThumbLeft, @NonNull Thumb mThumbRight, float dx, boolean isLeftMove) {
        if (isLeftMove && dx < 0) {
            if ((mThumbRight.getMPos() - (mThumbLeft.getMPos() + dx)) > mMaxWidth) {
                mThumbRight.setMPos(mThumbLeft.getMPos() + dx + mMaxWidth);
                setThumbPos(1, mThumbRight.getMPos());
            }
        } else if (!isLeftMove && dx > 0) {
            if (((mThumbRight.getMPos() + dx) - mThumbLeft.getMPos()) > mMaxWidth) {
                mThumbLeft.setMPos(mThumbRight.getMPos() + dx - mMaxWidth);
                setThumbPos(0, mThumbLeft.getMPos());
            }
        }
    }
    ...
    /*private int getUnstuckFrom(int index) {
        int unstuck = 0;
        float lastVal = mThumbs.get(index).getMVal();
        for (int i = index - 1; i >= 0; i--) {
            Thumb th = mThumbs.get(i);
            if (th.getMVal() != lastVal)
                return i + 1;
        }
        return unstuck;
    }*/
    ...
    private void calculateThumbValue(int index) {
        if (index < mThumbs.size() && !mThumbs.isEmpty()) {
            ...
            th.setMVal(pixelToScale(index, th.getMPos()));
            onSeek(this, index, th.getMVal());
        }
    }
    ...
    private void calculateThumbPos(int index) {
        if (index < mThumbs.size() && !mThumbs.isEmpty()) {
            Thumb th = mThumbs.get(index);
            th.setMPos(scaleToPixel(index, th.getMVal()));
        }
    }

    private float getThumbValue(int index) {
        return mThumbs.get(index).getMVal();
    }

    public void setThumbValue(int index, float value) {
        mThumbs.get(index).setMVal(value);
    ...
    private int getClosestThumb(float coordinate) {
        ...
        if (!mThumbs.isEmpty()) {
            for (int i = 0; i < mThumbs.size(); i++) {
                ...
                final float tcoordinate = mThumbs.get(i).getMPos() + mThumbWidth;
                if (coordinate >= mThumbs.get(i).getMPos() && coordinate <= tcoordinate) {
                    closest = mThumbs.get(i).getMIndex();
                }
            }
        }
        return closest;
    ...
    private void drawThumbs(@NonNull Canvas canvas) {
        ...
        if (!mThumbs.isEmpty()) {
            for (Thumb th : mThumbs) {
                if (th.getMIndex() == 0) {
                    canvas.drawBitmap(th.getMBitmap(), th.getMPos() + getPaddingLeft(), getPaddingTop() + mHeightTimeLine, null);
                } else {
                    canvas.drawBitmap(th.getMBitmap(), th.getMPos() - getPaddingRight(), getPaddingTop() + mHeightTimeLine, null);
                }
            }
        }
    }
