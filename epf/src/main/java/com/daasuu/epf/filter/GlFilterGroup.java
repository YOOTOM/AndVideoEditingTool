...
public class GlFilterGroup extends GlFilter2 {

    private final Collection<GlFilter2> filters;

    private final ArrayList<Pair<GlFilter2, EFramebufferObject>> list = new ArrayList<Pair<GlFilter2, EFramebufferObject>>();

    public GlFilterGroup(final GlFilter2... glFilters) {
        this(Arrays.asList(glFilters));
    }

    public GlFilterGroup(final Collection<GlFilter2> glFilters) {
        filters = glFilters;
    }
    ...
    public void setup() {
        ...
        if (filters != null) {
            ...
            for (final GlFilter2 shader : filters) {
                shader.setup();
    ...
    @Override
    public void release() {
        for (final Pair<GlFilter2, EFramebufferObject> pair : list) {
            if (pair.first != null) {
                pair.first.release();
            }
            if (pair.second != null) {
